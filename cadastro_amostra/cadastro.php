...
<div class="container">
    <div class="sidebar">
        ...
        <a href="#">Agenda</a>
        <a href="pagmedica.html">Voltar</a>
    </div>
    <div class="content">
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            $cadastro_tipo = $_POST["cadastro_tipo"]; // Tipo de cadastro: amostra ou exame
                $cadastro_tipo = $_POST["cadastro_tipo"]; // Tipo de cadastro: amostra ou exame
                // Ajusta o SQL de acordo com o tipo de cadastro
            if ($cadastro_tipo == "amostra") {
                $sql = "INSERT INTO amostras (...)";
            } else if ($cadastro_tipo == "exame") {
            ...
            $sql = "INSERT INTO amostras (sample_id, sample_type, collection_datetime, collection_location, patient_name, birthdate, gender, contact_telefone, contact_email, requester_name, requester_council, requester_council_number, requester_institution, analysis_type, request_datetime, responsible_lab, analysis_status, additional_notes) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

            if ($stmt = $conn->prepare($sql)) {
                // Vincula parâmetros à declaração
                $stmt->bind_param("ssssssssssssssssss", $sample_id, $sample_type, $collection_datetime, $collection_location, $patient_name, $birthdate, $gender, $contact_telefone, $contact_email, $requester_name, $requester_council, $requester_council_number, $requester_institution, $analysis_type, $request_datetime, $responsible_lab, $analysis_status, $additional_notes);
        ...
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="POST">
            ...
            <label for="cadastro_amostra">Cadastro de Amostra:</label>

            <div class="radio-group">
                <label>Tipo de Cadastro:</label>
                <input type="radio" id="cadastro_amostra" name="cadastro_tipo" value="amostra" checked>
                <label for="cadastro_amostra">Cadastro de Amostra</label>
                <input type="radio" id="cadastro_exame" name="cadastro_tipo" value="exame">
                <label for="cadastro_exame">Cadastro de Exame</label>
            </div>
            
            <label for="sample_id">Identificador da Amostra:</label>
            <input type="number" id="sample_id" name="sample_id" pattern="[0-9]+" placeholder="Código único da amostra" required>
            <br>
            <br>
            ...
            <label for="contact_telefone">Telefone do Paciente:</label>
            <input type="tel" id="contact_telefone" name="contact_telefone" pattern="[0-9]{10,11}" placeholder="(99) 9 9999-9999" required>
            <br>
            <br>
            
            <label for="contact_email">Email do Paciente:</label>
            <input type="email" id="contact_email" name="contact_email" placeholder="paciente@gmail.com">
            

            <h2>Informações do Solicitante</h2>
            <label for="requester_name">Nome do Solicitante:</label>
            <input type="text" id="requester_name" name="requester_name" placeholder="Nome do profissional de saúde que solicitou a análise" required>
            ...
            <input type="number" id="requester_council_number" name="requester_council_number" pattern="[0-9]+" placeholder="Número do Conselho" required>
            <br>
            <br>
